...
/**
 * Created by lex on 2016/7/4.
 */
public class DownloadThread extends Thread {

    private static Logger log = LogManager.getLogger(DownloadThread.class);
    ...
    public DownloadThread(int i, String url, String fileName, int start, int end, CountDownLatch downLatch) {
    ...
    public void run() {
        try {
            ...
            log.info(this.getName() + " 线程耗时： " + (d2.getTime() - d.getTime()) / 1000 + " 秒,实际共下载：" + l + "字节");//子线程下载耗时（秒）
            downLatch.countDown();
